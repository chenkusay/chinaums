...
Vue.component('city', {
    template: '<div id="cityChoose" v-show="" class="zent-portal zent-popover__region-select zent-popover zent-popover-internal-id-2 zent-popover-position-bottom-left"> <div data-reactroot="" class="zent-popover-content"> <div class="zent-region-select__popup"> <div class="zent-tabs zent-region-tabs"> <div class="zent-tabs-nav zent-tabs-size-normal zent-tabs-type-normal zent-tabs-align-left"> <div class="zent-tabs-nav-content"><!-- react-text: 2780 --><!-- /react-text --> <div class="zent-tabs-scroll"> <div class="zent-tabs-tabwrap" role="tablist"> <span class="zent-tabs-nav-ink-bar"></span> <div> <div role="tab" aria-labelledby="zent-tabpanel-68-1" class="zent-tabs-tab" :class="{\'zent-tabs-actived\': curSelectType == 1}" @click.stop="provinceTabDidClick"> <div class="zent-tabs-tab-inner">省份</div> </div> <div role="tab" aria-labelledby="zent-tabpanel-68-2" class="zent-tabs-tab" v-show="showCity" :class="{\'zent-tabs-actived\': curSelectType == 2}" @click.stop="cityTabDidClick"> <div class="zent-tabs-tab-inner">城市</div> </div> <div role="tab" aria-labelledby="zent-tabpanel-68-3" class="zent-tabs-tab" v-show="showCounty" :class="{\'zent-tabs-actived\': curSelectType == 3}" @click.stop="countyTabDidClick"> <div class="zent-tabs-tab-inner">县区</div> </div> </div> </div> </div> </div> </div> <div class="zent-tabs-panewrap"> <div class="zent-tab-tabpanel "> <div class="zent-region-list"> <span class="zent-region-list__item" v-for="item in show_cities"><span class="zent-region-list__item-link" :class="{active: item.selected}" @click.stop="cityDidClick(item)">{{item.n}}</span></span> </div> </div> </div> </div> </div> </div> </div>',
    data: function() {
        return {
            cities: [
                {
            ...
            showCounty: false,      // 是否显示县
            selectProvince: this.backProvince,   // 选择的省
            selectCity: this.backCity,       // 选择的市
            selectCounty: this.backCounty,     // 选择的县
            curSelectType: CityType.Province,

        }
    },
    props: ['backProvince', 'backCity', 'backCounty'],
    methods: {
        cityDidClick: function(item) {
            item.selected = true;
            switch (this.curSelectType) {
                case CityType.Province:
                    this.selectProvince = item;
                    this.showCity = true;
                    this.show_cities = this.getCities(item);
                    this.curSelectType = CityType.City;
                    this.selectCounty = null;
                    this.selectCity = null;
                    this.$emit('backchange', 0, item.n);
                    break;
                case CityType.City:
                    this.selectCity = item;
                    this.$emit('backchange', 1, item.n);
                    var counties = this.getCounties(item);
                    if (counties) {
                        this.show_cities = counties;
                        this.showCounty = true;
                        this.curSelectType = CityType.County;
                    }else {
                        this.show_cities = this.getCities(this.selectProvince);
                        this.$emit('closecity');
                    }
                    this.selectCounty = null;

                    break;
                case CityType.County:
                    this.selectCounty = item;
                    console.log(this.selectCounty.n)
                    this.$emit('backchange', 2, item.n);
                    this.show_cities = this.getCounties(this.selectCity);
                    this.$emit('closecity');
                    break;
            }
            this.cityStr();
        ...
        getProvinces: function() {
            var _this = this;
            return this.cities.map(function(p) {
                var selected = _this.selectProvince ? (p.n == _this.selectProvince.n) : false;
                return {n: p.n, selected: selected};
            });
        ...
        getCities: function(province) {
            var cities = null;
            var _this = this;
            this.cities.forEach(function(p) { if (p.n == province.n) return (cities = p.s.map(function(e) {
                var selected = _this.selectCity ? (e.n == _this.selectCity.n) : false;
                return {n: e.n, selected: selected};
            })); });
            return cities;
        },
        getCounties: function(city) {
            var couties = null;
            var _this = this;
            this.cities.forEach(function(p) { if (p.n == _this.selectProvince.n) p.s.forEach(function(p2) { if (p2.n == city.n) { return p2.s && (couties = p2.s.map(function(e) {
                    var selected = _this.selectCounty ? (e.n == _this.selectCounty.n) : false;
                return {n: e.n, selected: selected};
            })) } })});
            return couties;
        },
        provinceTabDidClick: function() {
            this.show_cities = this.getProvinces();
            this.curSelectType = CityType.Province;
            this.showCity = false;
            this.showCounty = false;
        },
        cityTabDidClick: function() {
            this.show_cities = this.getCities(this.selectProvince);
            this.curSelectType = CityType.City;
            this.showCounty = false;
        },
        countyTabDidClick: function() {
            this.show_cities = this.getCounties(this.selectCity);
            this.curSelectType = CityType.County;
        },
        cityStr: function() {
            var province = this.selectProvince ? this.selectProvince.n : '请选择省';
            var city = this.selectCity ? this.selectCity.n : '市';
            var county = this.selectCounty ? this.selectCounty.n : '县';
            this.$emit('citychange', province + '/' + city + '/' + county);
        }
    },
    mounted: function() {
        this.show_cities = this.getProvinces();
    ...
    watch: {
        backProvince: function (province) {
            console.log(province);
            this.selectProvince = {n: province};
            // this.cityStr();
            this.curSelectType = CityType.Province
            this.cityDidClick(this.selectProvince)

        },
        backCity: function (city) {
            this.selectCity = {n: city};
            // this.cityStr();
            this.cityDidClick(this.selectCity)
        },
        backCounty: function(county) {
            this.selectCounty = {n: county};
            // this.cityStr();
            console.log('hhhh' + county)
            this.cityDidClick(this.selectCounty)
        }
    }
})
